...
def register():
    ...
    if form.validate_on_submit(): #유효성 검사. 내용 채우지 않은 항목이 있는지까지 체크
        ...
        #usertable.userid = form.data.get('axieid')
...
def login():
    form = LoginForm(request.form) #로그인폼
    #userid = request.form['userid']
    
    if form.validate_on_submit(): #유효성 검사
        print('{}logged in'.format(form.data.get('userid')))
    ...
    #return 'Axie ID {}, welcome!'.format(form.data.get(userid)) and render_template('login.html', form = form)
